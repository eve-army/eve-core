...
- **Ops dashboard (optional second hostname):** Add another **A** record (e.g. host **`ops`** → same VPS IP) so **`ops.live.eve.army`** resolves. Copy the second `server` block in [`deploy/nginx-eve-core.conf`](./nginx-eve-core.conf) and set `YOUR_OPS_DOMAIN` to that name. Request TLS for both names in one cert: `sudo certbot --nginx -d live.eve.army -d ops.live.eve.army`. The app is the same process; open **`https://ops.live.eve.army/ops`** for Privy login, then **`/ops/console`**. In `/etc/eve-core/app.env` set **`NEXT_PUBLIC_PRIVY_APP_ID`**, **`PRIVY_APP_SECRET`**, **`PRIVY_JWT_VERIFICATION_KEY`** (from [Privy dashboard](https://dashboard.privy.io)), **`OPS_ADMIN_SOLANA_WALLETS`** (comma-separated Solana **base58** addresses that must appear as linked wallets on the Privy user), and **`OPS_SESSION_SECRET`** (16+ random characters). Then `sudo systemctl restart eve-core`. For durable multi-stream state on restart, set **`DATABASE_URL`** (PostgreSQL); otherwise streams/telemetry are kept in process memory only.
